...
PaddleOCR-VL consists of four core components: a layout detection model, element cropping, a VLM, and a post-processing module. The simplified workflow is illustrated as follows:
...
In this pipeline, the layout detection model takes the entire image as input and localizes various layout elements (e.g., tables and formulas). Based on the detection results, the system then crops the original image to generate sub-images, each containing a single layout element. These sub-images are fed into the VLM to produce corresponding recognition results (e.g., Markdown text). Finally, a post-processing module merges the outputs of all elements to produce the complete parsing result of the entire image. Therefore, **to fully leverage the capabilities of PaddleOCR-VL, it is necessary to adopt the complete pipeline that integrates layout detection, element cropping, the VLM, and post-processing, rather than using the VLM component alone.** This distinction will be referenced multiple times in the following sections, so it is important to clearly differentiate between the full PaddleOCR-VL pipeline and its VLM component.Taking PaddleOCR-VL v1 as an example, the layout detection model is PP-DocLayoutV2, and the VLM component is PaddleOCR-VL-0.9B. It is important to note that “PaddleOCR-VL-0.9B” does not represent a standalone variant of PaddleOCR-VL, but rather refers specifically to the VLM component within the complete PaddleOCR-VL v1 pipeline. This differs from common naming conventions of LLMs/VLMs—for instance, Qwen2-72B typically denotes a specific model variant within the Qwen2 series.
